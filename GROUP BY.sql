-- GROUP BY --
/*
Group by cümleciği bir lecet ifadesinde satırları, sutunları 
...
group by isim;--burada isim isim gruplayıp bize döndürdü
...
select urun_adi,min(urun_miktari)as mininmum_urun_miktari,max(urun_miktari)as maximum_urun_miktari from manav
where urun_adi is not null
group by urun_adi order by maximum_urun_miktari;
...
select ulke ,round(avg(maas),2) as ortalama
...
select ulke,sehir,count(sehir)as_calisan_sayisi from personel
group by ulke, sehir;
...
select ulke,cinsiyet,count(*),round(avg(yas))as yas_ortalamasi from personel
where maas>30000
group by ulke,cinsiyet;
...
select ulke,cinsiyet,count(ulke),round(avg(yas))as yas_ortalamasi from personel
where maas >30000
group by ulke ,cinsiyet
